...
{
    public partial class Rectangle : MyOrg.Models.JsonSystemText.Quadrilateral, IRectangle, IEquatable<Rectangle>
    {
        // Derived entities: 0

        protected override int OnGetEntityId() => 7;

        private static Rectangle CreateEmpty()
        {
            var empty = new Rectangle();
            empty.Freeze();
            return empty;
        }
        private static readonly Rectangle _empty = CreateEmpty();
        public static new Rectangle Empty => _empty;

        public new static Rectangle CreateFrom(Rectangle source)
        {
            if (source.IsFrozen) return source;
            return source switch
            {
                _ => new MyOrg.Models.JsonSystemText.Rectangle(source)
            };
        }

        public new static Rectangle CreateFrom(MyOrg.Models.IRectangle source)
        {
            if (source is Rectangle concrete && concrete.IsFrozen) return concrete;
            return source switch
            {
                _ => new MyOrg.Models.JsonSystemText.Rectangle(source)
            };
        ...
        protected override IEntityBase OnPartCopy() => new Rectangle(this);

...
namespace DTOMaker.SrcGen.Core
{
    public abstract class SourceGeneratorBase : IIncrementalGenerator
    {
        ...
        private static ParsedEntity GetParsedEntity(GeneratorAttributeSyntaxContext ctx)
        {
            ...
            return new ParsedEntity(fullname, entityId, members.ToArray(), generatedNamespace);
        }
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            // add base entity
            parsedEntities = parsedEntities.Collect().Select((list, _) =>
            {
                if (list.Length > 0)
                {
                    var baseEntity = new ParsedEntity("DTOMaker.Runtime.IEntityBase", 0, [], "DTOMaker.Runtime");
                    var newList = new List<ParsedEntity> { baseEntity };
            ...
            }).SelectMany((list, _) => list.ToImmutableArray());
            ...
            IncrementalValuesProvider<OutputEntity> outputEntities = parsedEntities.Combine(parsedMembers.Collect())
                .Select((pair, _) =>
                {
                    ...
                    return new OutputEntity(entity.NameSpace, entity.FullName, entity.IntfName, entity.EntityId, 
                        members.ToArray());
                });

...
Emit("            {");
                ...
                using var _ = NewScope(derived);
Emit("                T_NameSpace_.JsonSystemText.T_EntityImplName_ source2 => new T_NameSpace_.JsonSystemText.T_EntityImplName_(source2),");
                }
Emit("                _ => new T_NameSpace_.JsonSystemText.T_EntityImplName_(source)");
Emit("            };");
Emit("        }");
Emit("");
...
Emit("            {");
                foreach(var derived in entity.DerivedEntities.OrderByDescending(e => e.ClassHeight)) {
                using var _ = NewScope(derived);
Emit("                T_NameSpace_.IT_EntityIntfName_ source2 => new T_NameSpace_.JsonSystemText.T_EntityImplName_(source2),");
                }
Emit("                _ => new T_NameSpace_.JsonSystemText.T_EntityImplName_(source)");
Emit("            };");
Emit("        }");
